...
class _AdminLeaveApprovalScreenState extends State<AdminLeaveApprovalScreen> {
  ...
  Future<void> _processLeave(bool approved) async {
    ...
    setState(() {
      _isProcessing = true;
      _statusMessage = approved ? 'Approving leave request...' : 'Declining leave request...';
    });
    ...
    try {
      ...
      final profileSnap = await _firestore.collection('employee_profiles').doc(widget.employeeId).get();
      ...
      final usersQuery = await _firestore.collection('users').where('employeeId', isEqualTo: widget.employeeId).limit(1).get();
      String? employeeUid;
      ...
      final leaveRef = _firestore.collection('leave_requests').doc(leaveDocId);
      batch.set(leaveRef, {
      ...
      final attendanceRef = _firestore.collection('attendance').doc(leaveDocId);
      batch.set(attendanceRef, {
        ...
        'attendanceStatus': approved ? 'approved_leave' : 'not_considered_attendance',
        'dayStatus': approved ? 'approved_leave' : 'not_considered',
        'leaveStatus': approved ? 'approved_leave' : 'rejected_leave',
        'reason': adminReason.isEmpty ? (approved ? 'Approved by Admin' : 'Declined by Admin') : adminReason,
      ...
      if (employeeUid != null && employeeUid.isNotEmpty) {
        final nowIstStr = DateTime.now().toUtc().add(const Duration(hours: 5, minutes: 30)).toIso8601String();
        ...
        final msgRef = _firestore.collection('team_messages').doc();
        batch.set(msgRef, notifyMsg);

        // Add to fcm_outbox to deliver the real push notification
        final outboxRef = _firestore.collection('fcm_outbox').doc(msgRef.id);
        batch.set(outboxRef, {
          ...notifyMsg,
          'messageId': msgRef.id,
          'status': 'pending',
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            approved ? 'Leave request approved successfully.' : 'Leave request declined.',
          ),
          backgroundColor: approved ? IndustrialColors.secondary : IndustrialColors.error,
        ),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return AppShell(
      ...
      child: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        stream: _firestore.collection('leave_requests').doc(leaveDocId).snapshots(),
        builder: (context, leaveSnap) {
          ...
          final employeeName = leaveData['employeeName'] as String? ?? 'Employee';
          final requestedDate = leaveData['date'] as String? ?? widget.date;
          final empReason = leaveData['reason'] as String? ?? 'No reason provided';
          final currentStatus = leaveData['status'] as String? ?? 'requested_leave';
          ...
          return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
            ...
            builder: (context, profileSnap) {
              ...
              final joiningDate = profile['joiningDate'] as String? ?? 'Not specified';
              final department = profile['department'] as String? ?? 'TECH';
              final role = profile['role'] as String? ?? 'EMPLOYEE';
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              const Text(
                                'Joining Date:',
                                style: TextStyle(color: IndustrialColors.onSurfaceVariant, fontSize: 13),
                              ),
                              Text(
                                joiningDate,
                        ...
                        children: [
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              StatusChip(
                                label: currentStatus.toUpperCase().replaceAll('_', ' '),
                      ...
                      children: [
                        Expanded(
                          child: PrimaryActionButton(
                            ...
                            onPressed: _isProcessing ? null : () => _processLeave(false),
                          ),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                          child: PrimaryActionButton(
                            ...
                            onPressed: _isProcessing ? null : () => _processLeave(true),
                          ),
                        ),
                      ],
                    ),
